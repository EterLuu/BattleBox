...
public class AdminCommand extends BaseSubCommand {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args[0].equals("start")) {
            ...
            if (area == null) {
                sender.sendMessage(Utils.translateColorCodes("&c[Battle Box] &b错误的场地。"));
                return true;
            }

            if (teamCard1 == null || teamCard2 == null || teamCard1.equals(teamCard2)) {
                sender.sendMessage(Utils.translateColorCodes("&c[Battle Box] &b错误的队伍。"));
                return true;
            }
            ...
            if (AreaManagerTask.teamJoin(queueTeam, area)) {
                sender.sendMessage(Utils.translateColorCodes("&c[Battle Box] &b调度成功。"));
            } else {
                sender.sendMessage(Utils.translateColorCodes("&c[Battle Box] &b调度失败。"));
            }
