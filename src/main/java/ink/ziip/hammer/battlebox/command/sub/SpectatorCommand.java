...
public class SpectatorCommand extends BaseSubCommand {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (area == null) {
            user.sendMessage("&b[Battle Box] &c错误的场地名。");
            return true;
        }

        if (user.getPlayer().getGameMode() == GameMode.SPECTATOR) {
            user.sendMessage("&b[Battle Box] &c你已经在旁观一个游戏了，使用“/battlebox leave”退出旁观。");
            return true;
        }
        ...
        if (team != null) {
            ...
            if (teamCard != null && teamCard.getArea() != null) {
                user.sendMessage("&b[Battle Box] &c你加入一个游戏了，不能旁观。");
                return true;
            }
        }

        if (area.joinAsSpectator(user)) {
            user.sendMessage("&b[Battle Box] &c旁观场地中。");
        } else {
            user.sendMessage("&b[Battle Box] &c旁观场地失败。");
        }
